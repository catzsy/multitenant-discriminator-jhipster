...
export class TenantComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.tenantService
      ...
      .subscribe(
        (res: HttpResponse<ITenant[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInTenants(): void {
    this.eventSubscriber = this.eventManager.subscribe('tenantListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: ITenant[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
